...
def plot_and_save_roc(y_true, y_score, output_path, beta_cut=None, n_events=None):
    ...
    if operating_point is not None:
        annotation_lines.extend([
            rf"$TPR_\beta$ = {operating_point[1]:.3f}",
            rf"$FPR_\beta$ = {operating_point[0]:.3f}",
        ])
    ax.text(
        0.75,
        0.25,
    ...
    hep.cms.label(ax=ax, data=False, text="Preliminary", rlabel=r"Phase-2 (14 TeV)")
    ax.text(0.2, 0.97, r"$t\bar{t}$, $\langle\mu\rangle=200$", transform=ax.transAxes, fontsize=20, verticalalignment='top', horizontalalignment='right')
...
def plot_score_distributions(y_true, y_score, output_path, beta_cut=None):
    ...
    bins = [0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1.0]
    ...
    ax.tick_params(axis="both", labelsize=20)
    ax.grid(True, alpha=0.25)
    ax.legend(loc="upper center", fontsize=20, frameon=False, ncol=1)
